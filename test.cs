...
class Test {
	...
	public void Count() {
		...
		Debug.Assert(count == 0, $"count 0. found: {count}, expected: 0");

		list.PushFront(1);
		count = list.Count();
		Debug.Assert(count == 1, $"count 1. found: {count}, expected: 1");

		list.PushFront(2);
		list.PushFront(3);
		count = list.Count();
		Debug.Assert(count == 3, $"count 3. found: {count}, expected: 3");
	}
}
